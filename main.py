...
class CountHandler(webapp2.RequestHandler):
    def get(self):
		my_var={"numbers":[9,5,5,10]}
		count_template = my_env.get_template('templates/count.html')
		self.response.write(count_template.render(my_var))
class PigHandler(webapp2.RequestHandler):
	def get(self):
		new_phrase={"translated_word": "NONE"}
		pig_template = my_env.get_template('templates/pig.html')
		self.response.write(pig_template.render(new_phrase))
		
	def post(self):
		word=self.request.get('user_word')
		def pigLatin(word):
			vowels = ["a", "e", "i", "o", "u"]
		...
		pl_result = pigLatin(word)
		new_phrase={"translated_word": pl_result}
		pig_template = my_env.get_template('templates/pig.html')
		self.response.write(pig_template.render(new_phrase))
app = webapp2.WSGIApplication([
    ('/hello', MainHandler),
	('/count',CountHandler),
	('/PigHandler',PigHandler)
], debug=True)
